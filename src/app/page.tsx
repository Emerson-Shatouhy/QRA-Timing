...
import { Calendar, MapPin, Trophy, Users, ChevronRight, Plus } from 'lucide-react';
...
export default function Home() {
  ...
  function RegattaCard({ regatta }: { regatta: RegattaWithStats }) {
    ...
    return (
      <Link href={`/regatta/${regatta.id}`} className="block group">
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-200">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1 min-w-0">
              ...
              {regatta.description && (
                <p className="text-sm text-gray-500 mt-1 line-clamp-1">{regatta.description}</p>
              )}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-gray-600">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>{regatta.raceCount} {regatta.raceCount === 1 ? 'event' : 'events'}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Users className="w-4 h-4 text-gray-400" />
              <span>{regatta.entryCount} {regatta.entryCount === 1 ? 'entry' : 'entries'}</span>
            </div>
          </div>
        </div>
      </Link>
          ...
          <div className="space-y-8">
            ...
            {upcoming.length > 0 && (
              <section>
                ...
                <div className="grid gap-4">
                  {upcoming.map(r => <RegattaCard key={r.id} regatta={r} />)}
                </div>
              </section>
            )}
            ...
            {completed.length > 0 && (
              <section>
                ...
                <div className="grid gap-4">
                  {completed.map(r => <RegattaCard key={r.id} regatta={r} />)}
                </div>
              </section>
            )}
          </div>
