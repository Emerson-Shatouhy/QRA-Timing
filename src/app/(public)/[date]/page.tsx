...
export default async function SpectatorDatePage({ params }: PageProps) {
  ...
  races?.forEach((race: any) => {
    if (race.host_team_id && !seenHosts.has(race.host_team_id)) {
      ...
      hostTeams.push({ id: race.host_team_id, name: race.host_team?.team_name || "Unknown" });
    }
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 py-6">
        <Link href="/" className="text-sm text-slate-500 hover:text-slate-900 mb-4 inline-flex items-center gap-1 transition-colors">
          &larr; All Dates
        </Link>

        <h1 className="text-2xl font-bold text-slate-900 mb-1">{displayDate}</h1>
        <p className="text-slate-500 mb-6 text-sm">
          {races?.length || 0} race{(races?.length || 0) !== 1 ? "s" : ""} on Lake Quinsigamond
        </p>
        ...
        {!races || races.length === 0 ? (
          ...
          <SpectatorRaceGrid races={races as any[]} hostTeams={hostTeams} date={date} />
        )}
      </div>
    </div>
  );
