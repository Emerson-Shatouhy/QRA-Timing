...
export default function RaceTimingPage() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setAllTaps((prev) => {
            if (payload.eventType === 'INSERT') return [payload.new as RawTiming, ...prev];
          ...
          setMyTaps((prev) => {
            if (payload.eventType === 'INSERT') {
              ...
              if (t.timer_user_id === currentUserId && t.bow_number === null) return [t, ...prev];
            }
            if (payload.eventType === 'UPDATE') {
              const t = payload.new as RawTiming;
  ...
  const handleTap = async () => {
    ...
    if (!result) {
      ...
      alert('Failed to record tap. Check your connection.');
      return;
    }
  ...
  const raceElapsed = race?.actual_start && !isHeadRace
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-5">
      ...
      {(isStarted || anyEntryStarted) && (
        <div className="bg-white rounded-lg border p-4 space-y-4">
          ...
          <button
            onClick={handleTap}
            className="w-full h-28 rounded-xl bg-blue-600 hover:bg-blue-700 active:bg-blue-900
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {sortedResults.map((r, i) => (
                <tr key={r.entry_id} className={r.has_outlier_flag ? 'bg-yellow-50' : ''}>
